...
const initGameFunctionality = () => {
  const gameWindow = document.querySelector('.b-game__window');
  const gamePlayBtn = document.querySelector('.b-game__start-btn');
  const gamePlayBtnTxt = gamePlayBtn.textContent;
  const gamePlayBtnTxtSub = 'play another';
  const gamePlayBtnTxtContinue = 'continue';
  ...
  const gameIframeDataSandbox = gamePlayBtn.getAttribute('data-sandbox');
  const gameIframeSandbox = gameIframeDataSandbox == 1 ? 'allow-same-origin allow-scripts allow-modals' : '';
  ...
  const createGameIframe = () => {
    ...
    gameIframeCreated.setAttribute('sandbox', gameIframeSandbox);
